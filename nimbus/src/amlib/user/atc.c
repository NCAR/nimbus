...
static NR_TYPE
	UPFCTR	= 0.999444,
	TAUP	= 30.0,	// Very original value was 150.
	TAU	= 30.0;
...
void initATC(DERTBL *varp)
{
  ...
  if ((tmp = GetDefaultsValue("TEMP_TAUP", varp->name)) == NULL)
  {
    sprintf(buffer, "TEMP_TAUP set to %f in AMLIB function satc.\n", TAUP);
    LogMessage(buffer);
  }
  else
    TAUP = tmp[0];

  if ((tmp = GetDefaultsValue("TEMP_TAU", varp->name)) == NULL)
  {
    sprintf(buffer, "TEMP_TAU set to %f in AMLIB function satc.\n", TAU);
    LogMessage(buffer);
  }
  else
    TAU = tmp[0];
