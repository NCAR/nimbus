...
WDOG_ID wid;
int new_sec  = 0;
int ticks25 = sysClkRateGet() / RATE_25;	// ticks per 25 hz interval
...
// Get the operating parameters. 
  ...
  printf ("0,1        0,1,2     1,5,25      16,32,64   0-3        y,n\n");
...
// Check the user entry.
  ...
  if ((rate != RATE_1) && (rate != RATE_5) && (rate != RATE_25)) {
...
// Start up a watchdog timer at a 25 hertz interval. 
  wid = wdCreate();              
  wdStart ((WDOG_ID)wid, ticks25, (FUNCPTR)v1d_wdisr, FALSE);
...
// Call the vme 1d sampling isr.
  pms1.pms1VmeIsr(hz25_cnt);

  hz25_cnt = (hz25_cnt + 1) % RATE_25;
}
/*****************************************************************************/
