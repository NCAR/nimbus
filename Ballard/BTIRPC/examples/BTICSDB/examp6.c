...
/**
*
*  BTICSDB DRIVER EXAMPLE 6  (10/23/2009)
...
*  ALL RIGHTS RESERVED
*
*  NAME:   EXAMP6.C -- Transmit and Receive List Buffer (non-continuous) example
*
**/
...
*  list buffers with non-continuous messages that are
...
**/
	...
	xmtmsg	= BTICSDB_MsgCreate(MSGCRTCSDB_DEFAULT | MSGCRTCSDB_NONCONT,hCore);
...
*  transmit data as long as it is valid.
*
...
**/
	...
	BTICSDB_MsgValidSet(xmtmsg,hCore);      //must set a non-continuous message valid for xmt
...
**/
	...
	consoleprintf("\nPress escape to exit or any other key to trigger a non-continuous transmission.\n\n");
	...
	while(!done)
	{
		...
		if(BTICSDB_ListStatus(rcvlistaddr,hCore) != STAT_EMPTY)
		{
			...
			consoleprintf("\rAddress=%02X   Data=%02X   Bytes=%d",rcvbuf[0],rcvbuf[1],count);
		}
	}
