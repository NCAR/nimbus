...
dirname='C:\Users\kaser\Documents\CONTRAST\matlab\RF10\resultsRF10\';
...
if RF10_forICARTT.TIME(1)<43200 
date_start=datevec(RF10_forICARTT.DOY(1)+datenum('12-31-2013')+1);%% add 1 for the day if take off is after Midnight UTC (==10AM in Guam)
else
date_start=datevec(RF10_forICARTT.DOY(1)+datenum('12-31-2013')-1);%% add 1 for the day if take off is after Midnight UTC (==10AM in Guam)    
end
...
conc_data=[RF10_forICARTT.CO_PPBV,RF10_forICARTT.CO2_dry_ppm,RF10_forICARTT.CH4_dry_ppm];
...
figure()
subplot(3,1,1)
plot(utc,RF10_forICARTT.CO_PPBV,'g')
legend('CO [ppbv]')
grid on
xlabel('utc')
ylabel('ppbv')
xlim([utc(1) utc(end)])
ylim([min(RF10_forICARTT.CO_PPBV) max(RF10_forICARTT.CO_PPBV)])
subplot(3,1,2)
plot(utc,RF10_forICARTT.CO2_dry_ppm)
legend('CO2 [ppmv]')
grid on
xlabel('utc')
ylabel('ppmv')
xlim([utc(1) utc(end)])
ylim([min(RF10_forICARTT.CO2_dry_ppm) max(RF10_forICARTT.CO2_dry_ppm)])
subplot(3,1,3)
plot(utc,RF10_forICARTT.CH4_dry_ppm,'r')
legend('CH_4 [ppmv]')
grid on
xlabel('utc')
ylabel('ppmv')
xlim([utc(1) utc(end)])
ylim([min(RF10_forICARTT.CH4_dry_ppm) max(RF10_forICARTT.CH4_dry_ppm)])
set(gcf, 'Color', 'white'); % white bckgr
screen_size = get(0, 'ScreenSize');
set(gcf, 'Position', [0 0 screen_size(3) screen_size(4) ] );
cd C:\Users\kaser\Documents\CONTRAST\matlab\RF10\resultsRF10\
export_fig( gcf, ...      % figure handle
    ['CO_CO2_CH4_RF10'],... % name of output file without extension
    '-painters', ...      % renderer
    '-pdf', ...           % file format
...
filenamexp=['C:\Users\kaser\Documents\CONTRAST\matlab\RF10\resultsRF10\COCO2CH4_GV_',date3,'_RA.ict'];

fid = eval(['fopen(''',filenamexp ''',''w+'')'])

fprintf(fid,'%s\n','35, 1001'); %change number of header lines
fprintf(fid,'%s\n','Campos, T.L.; Kaser, L.; Stell, M.; Aquino, J.; Allbee, D.; Munnerlyn, J.; Weinheimer, A.J.; Flocke, F.M;');
fprintf(fid,'%s\n','NCAR');
fprintf(fid,'%s\n','NCAR Aerolaser');
fprintf(fid,'%s\n','CONTRAST');
fprintf(fid,'%s\n','1, 1');%?
fprintf(fid,'%s\n',[date3(1:4),', ',date3(5:6),', ',date3(7:8),', ','2014, 02, 08']); %change dates: %UTC date when data begin, UTC date of data reduction or revision - comma delimited (yyyy, mm, dd, yyyy, mm, dd).
...
fprintf(fid,'%s\n','CO, ppmv'); % name of first species after the 3 time columns
fprintf(fid,'%s\n','CO2, ppmv'); 
fprintf(fid,'%s\n','CH4, ppmv'); 
...
fprintf(fid,'%s\n','UNCERTAINTY: CO: 3ppb +/-7%; CO2: 1.3%; CH4: 30ppb'); % change!!!
fprintf(fid,'%s\n','ULOD_FLAG: N/A');										
fprintf(fid,'%s\n','ULOD_VALUE: N/A');										
...
figure()
plotyy(utc,RF10_forICARTT.CO_PPBV,utc,O3_interp)
legend('CO','O3')
% xlim([utc(1) utc(end)])
grid on
set(gcf, 'Color', 'white'); % white bckgr
...
cd C:\Users\kaser\Documents\CONTRAST\matlab\RF10\resultsRF10\
export_fig( gcf, ...      % figure handle
    ['O3_CO_RF10'],... % name of output file without extension
    '-painters', ...      % renderer
    '-pdf', ...           % file format
    '-r300' );             % resolution in dpi
...
figure()
scatter(double(RF10_forICARTT.CO_PPBV),double(O3_interp),50,double(RF10_forICARTT.PALTF))
xlabel('CO [ppbv]')
ylabel('O_3 [ppbv]')
colorbar;
grid on
...
cd C:\Users\kaser\Documents\CONTRAST\matlab\RF10\resultsRF10\
export_fig( gcf, ...      % figure handle
    ['O3_CO_scatter_with_height_RF10'],... % name of output file without extension
    '-painters', ...      % renderer
    '-pdf', ...           % file format
    '-r300' );             % resolution in dpi
