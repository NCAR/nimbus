		...
		if ($#date+1 == 2) { 
			...
			if ($checkGnd) {
				...
				foreach my $file (@files) {
					$count++;
					if ($count % int(($#files+1)/20)  == 0) { print "\rScanning for ground images: " . int($count/($#files+1)*100 + .5) . "% Done"; }
					#if file time is older than landing time remove it
					if (IsOlder(DateFromFile($file),@{$date[1]})) { push(@filesToRemove, $file); splice(@files,$count-1, 1)} 
					#if file time is earlier than takeoff time remove it
					if (IsOlder(@{$date[0]},DateFromFile($file))) { push(@filesToRemove, $file); splice(@files, $count-1, 1)}
				}
...
if ($checkDark) {
	...
	foreach my $file (@files) {
		$count++;
		if ($count % int(($#files+1)/20)  == 0) { print "\rScanning for dark images: " . int($count/($#files+1)*100 + .5) . "% Done"; }
