...
if ( (scalar(@ARGV) != 2) || ($ARGV[0] eq "-h") ) {
	...
	print "USAGE: movie.pl  parameterFileName flight\n\n";
	exit;
	}
...
if ($cameraName =~ m/Axis/) {	# rescale to square pixels (width / 1.1) in approximate widescreen (16:9) 
    				# format and allowing room for annotation.
	if (!$scale) {$scale = "640x480!";}	# The '!' forces resize to exact dimensions, accounting for 
						# pixel aspect ratio.
...
$labelImage->Annotate( font=>$annotationFont, pointsize=>$fontSize, weight=>500, gravity=>'SouthWest',
    fill=>'black',text=>"$theText");
...
foreach $fileName (@jpegFiles) {	
	...
	if ($scale) {$inputImage->Scale( geometry=>$scale )};	# Sample is faster for simple sub-sampling.
	...
	while (substr($flightData[0],0,8) ne $imageTime_withColons)  {
