...
static char raw_in_file[file_length], raw_out_file_1[file_length],
	...
	*base_directory, *timeString, *dateString, *flightNumber;

static double	delay_time, plumb_time, tau_cnc, flow_settling_time,
		desmearing_accuracy, scan_time, time_step, temperature,
		pressure;

static double	counts_raw[num_bins], counts_inv[num_bins],
		counts_unsmeared[num_bins];
...
strcat(n_long_file_2, "out_to_rdmap");
...
static void read_in_data(double &t_total_up, double &t_total_down, int &total_bins_up, int &total_bins_down, double t_extr_up[2], double t_extr_down[2])
{
  ...
  if (param_in.is_open() == false)
  {
    cerr << "Unable to open " << param_in << endl;
    exit(1);
  }
  ...
  if (labview_par_in.is_open() == false)
  {
    cerr << "Unable to open " << labview_parameter_file << endl;
    exit(1);
  }
  ...
  if (matrix_par_in.is_open() == false)
  {
    cerr << "Unable to open " << matrix_parameter_file << endl;
    exit(1);
  }

  matrix_par_in >> t_total_up >> t_total_down >> t_extr_up[0] >> t_extr_up[1]
    >> t_extr_down[0] >> t_extr_down[1] >> delay_time >> num_pts_fit_low_Dp
    >> num_pts_fit_high_Dp;
...
//	counts_inv[num_bins], seconds_smeared, scan_num_double, cnc_conc;
	seconds_smeared, scan_num_double, cnc_conc;
...
// Why is first value a 2.0, then some 0.9999's?
//printf("%d %f %f\n", bin, counts_raw[bin], counts_unsmeared[bin]);
    }
...
//printf(" %d %f\n", bin, counts_inv[bin]);
  ...
  for (bin = 0; bin < total_bins; ++bin)
    if (fabs(counts_inv[bin]) < lowest_counts)
      counts_inv[bin] = 0.0;   
...
static void solve_for_n(double dia_from_inv[3][num_bins], double counts[num_bins], int bins, int extr_low_bins, int extr_high_bins, int scan_up, ofstream &nimbus_out)
{
  ...
  for (i = 0, bin = 1; bin < bins - 1; ++bin)
    ...
    if ((bin > extr_low_bins) && (bin < (bins - extr_high_bins - 1)))
      ...
      dlogd[i] = fabs(log10(dia_from_inv[2][bin]/dia_from_inv[0][bin]));

      total_conc += n_final[i] * dlogd[i];
  ...
  for (bin = 0; bin < i; ++bin)
    ...
    if (scan_up)
      {
      nimbus_out << (dia_final[bin] * 1.0e9) << " " << dlogd[bin] << " " << n_final[bin] << "\n";
//printf("%f %f %f\n", (dia_final[bin] * 1.0e9), dlogd[bin], n_final[bin]);
      }
    else
      {
      nimbus_out <<(dia_final[i-bin-1]*1.0e9) << " " << dlogd[i-bin-1] << " " << n_final[i-bin-1] <<"\n";
      }
    }
...
void rdma_inv(int argc, char *argv[])
{
  ...
  if (firstTime)
    {
    base_directory = getenv("DATA_DIR");
  ...
  ofstream	nimbus_out(n_long_file_2, ios::trunc);
