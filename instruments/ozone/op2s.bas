...
'Initialize A/D channels. This only needs to be done once.
...
DO
	...
	OUT CONT1%, 158         'Disarm and save #2, #3, #4, #5
	OUT CONT1%, 95          'Load all counters
...
'Removed from here to test if initializing once is sufficient
'OUT DIFFBASEADDR% + 2, 112       'I/O CHANNEL TO START AT 0 END 7 (70H)
'OUT DIFFBASEADDR% + 3, 6         'SET ANALOG INPUT RANGE TO GAIN = 4
...
LOOP UNTIL FIFO% = 0 'FIFO is automatically decremented by hardware when LSB and MSB are read out of it.
...
OPEN DATAOUT$ FOR APPEND AS #1
	PRINT #1, "COMP_OP2   CYCLE_No   CNT_A   CNT_B   PCHA    PCHB    PBAR    AIN     BIN     AOUT    BOUT"
CLOSE #1
