...
/*
** pack3f() - pack 3 words into 1.
*
*	Stephen O. Gombosi NCAR/SCD 11/06/2001
*/
static
void pack3f(
	double	*arn,
	...
	double		xmin	= apn[0];
	double		xsf 	= apn[1];
        unsigned int ihigh, imiddle, ilow = 0; /* high, middle, low scaled values */

#ifdef DEBUG
    fprintf(stderr,"Enter unpack3f, xmin = %g, xsf = %g\n",xmin,xsf);
#endif


	npn = *nrn/3; /* number of full sets of 3 to pack */

	/* Pack full sets of 3 words */

	for(j=0; j < npn; j++){

		i = 3 * j;

		/* Scale the floating point value into 3 unsigned ints */

		ihigh = (unsigned int) ((arn[i]   - xmin) * xsf);
		imiddle = (unsigned int) ((arn[i+1] - xmin) * xsf);
		ilow = (unsigned int) ((arn[i+2] - xmin) * xsf);

#ifdef DEBUG
     fprintf(stderr,"ihigh = %x(%d), imiddle = %x(%d), ilow = %x (%d)\n",
             ihigh, ihigh, imiddle, imiddle, ilow, ilow);
#endif

		/* Pack the scaled values */
		data.it[0] = ihigh<<10 | imiddle >>11;
		data.it[1] = imiddle<<21 | ilow;
		apn[j+2] = data.temp;
	}
...
#ifdef DEBUG
     fprintf(stderr,"ihigh = %x(%d), imiddle = %x(%d) \n",
             ihigh, ihigh, imiddle, imiddle );
#endif
...
void unpack3f(
	...
	int 	npn; /* number of packed "words"  */
	int 	remainder = *nrn % 3; /*number of values to be unpacked from last partial */
	int 	j, i;
	int	ihigh, imiddle, ilow; /*high, middle, and low scaled values */
	const int  mask10 = (1<<10) - 1;
	const int  mask21 = (1<<21) - 1;

	double	xmin, xsf;

	npn = *nrn/3;  /* get number of packed values */
	...
	for(j=0; j < npn; j++){
		...
		data.temp = apn[j+2];
		ihigh = data.it[0] >> 10;
		imiddle = ((data.it[0] & mask10)<< 11) |
                          (data.it[1] >> 21);
                ilow = data.it[1] & mask21;
     ...
     fprintf(stderr,"ihigh = %x(%d), imiddle = %x(%d), ilow = %x (%d)\n",
             ihigh, ihigh, imiddle, imiddle, ilow, ilow);
                ...
                arn[i] = (double) ihigh / xsf + xmin;
		arn[i+1] = (double) imiddle / xsf + xmin;
		arn[i+2] = (double) ilow / xsf + xmin;
...
void	unpack(
	...
	int	i;
		...
		case 2:
			...
			break;
		default:
			(void)fprintf(stderr, "packaf - packing density .ne. 2, 3,  or 4 - stop\n");
			exit(1);
	}
	/*
	** Error checks...
