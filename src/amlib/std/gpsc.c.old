...
INPUT:		LAT LON GLAT GLON VNS VEW GVNS GVEW ROLL GSTAT
...
DERTBL	*varp;		/* Information about current variable	*/
{
  ...
  if (FeedBack == LOW_RATE_FEEDBACK)
    ...
    if (gstat == 0x0b00)	/* 3 satallites is also considered good	*/
      gstat = 0;


    /* Bad Positions for using GPS?
     */
    if (gstat > 0 || fabs(glat) > 90.0 || fabs(glon) > 180.0 || fabs(roll) > ROLL_MAX)
      {
