...
const double R1_RDMA = 0.24;		// cm
const double R2_RDMA = 5.04;		// cm
const double B_RDMA = 1.0;		// cm
const double L_TSI_DMA = 44.44;		// cm
const double R1_TSI_DMA = 0.937;	// cm
const double R2_TSI_DMA = 1.958;	// cm
const double L_CIT_CDMA = 41.19;	// cm
const double R1_CIT_CDMA = 0.945;	// cm
const double R2_CIT_CDMA = 1.92;	// cm
const double L_nano_DMA = 5.0;
const double R1_nano_DMA = 0.94;
const double R2_nano_DMA = 1.91;
const double L_eff_TSI_DMA = 1.3;	// m             
const double pi = M_PI;
const double G_tilda_rdma = 2.92;
const double G_TSI_DMA = 4.25;
const double G_nano_DMA = 4.25;
const double k = 1.381e-23;		// J/K
const double e = 1.602e-19;		// C
const double ratio_of_pos_to_neg_ion_conc = 1.0;
const double ratio_of_pos_to_neg_ion_mob = 0.875;
const double ep0 = 8.854e-12;		// C^2/(N-m^2)
const double mfp_1_atm_298_K = 65.1e-9;	// m
const double Dp_min = 1.0e-9;		// m       // Used for dia calc from Zp
const double Dp_max = 2.0e-5;		// m                    
const int segments = 9;			// For averaging over dia bin
const double alpha_min = 1.0e-9;	// limit for inversion factor
const double omega_limit = 1.0e-5;	// limit for transfer function
const int num_pts_fit_low_Dp = 8;	// num pts used for extrapolation
const int num_pts_fit_high_Dp = 4;
const int num_bins = 200;		// num_bins must be >= actual number
const int num_extr_bins = 20;  // num_extr_bins >= extrapolated pts + pts used for extrapolation
const double inv_factor_ratio = 1.0e5;	// largest allowed ratio of two different inv(Dpi, countsj)
const int file_length = 100;		// number of characters in a file name
const int num_param = 17;		// num_param >= number of parameters stored
const int max_chg = 10;			// highest # charges considered
const double R = 8.314;			// J/mol-K
const double mobility_ratio_increment = 1.01;
const double unreasonable_number = -1.0e10;
const int max_lines = 25;
const double mw_air = 29.0;
const double mw_positive = 290;
const double mw_negative = 140;
const double Z_positive = 1.15e-4;
const double Z_negative = 1.425e-4;
const double max_charge_error = 4.0e-4;
const double kQ = 8.988e9;
const double very_large_number = 1.0e25;
const double very_small_number = 1.0e-25;

char	base_directory[1024];
char	tubing_lengths_file[file_length], first_time_tubing_file, x;

double	var[num_param];

/* -------------------------------------------------------------------- */
void getinfo(int &mat_num)
{
  double value[num_param];
  int	i, j, param_number, char_in_line, letter, get_single_length, lines;
  char	param_correct, log_file[file_length], log_save, mat_param[file_length],
  	comments[50], tubing_lengths_file_name[file_length], 
	multiple_lengths, mat_dir_char, mat_dir_char_str[2],
	mat_dir[file_length], parameter_file[file_length];


  strcpy(base_directory, getenv("NIMBUS"));
  ...
  ofstream log_out(log_file, ios::beg | ios::trunc);
...
//  cin>>multiple_lengths;
  ...
  ofstream mat_param_out(mat_param, ios::beg | ios::trunc);
...
/* -------------------------------------------------------------------- */
double get_diameter(double Zp, double pressure, double T_K)
{
...
/* -------------------------------------------------------------------- */
double get_omega(double Zp, double Zp_star, int chg, double Zp_ratio_low, 
	double Zp_ratio_high)
{
  double erff(double x);
  double beta, Qa, Qm, Qsh, Qe, delta, Zp_tilda, Pe_rdma, sigma_tilda, err1,
	err2, err3, err4, err3_4, x1, x2, x3, x4, omega, T_K;
  int	i, dma_type;
...
/* -------------------------------------------------------------------- */
double calc_res_time()
{
  double res_time, Qa, Qm, Qsh, Qe;
  int dma_type, i;
...
/* -------------------------------------------------------------------- */
void get_mobility_ratio(double &Zp_ratio_high, double &Zp_ratio_low, double T_K,
...
/* -------------------------------------------------------------------- */
void calc_mobility_bounds(double &Zp_low_v_max, double &Zp_high_v_min, int mat_num)
{
  int		i, dma_type;
  double	Zp_high_v, Zp_low_v, Zp_high_ratio, Zp_low_ratio,
...
/* -------------------------------------------------------------------- */
void get_effective_voltages(double Zp_ratio_high, double Zp_ratio_low, double v_combined_up[num_bins], double v_combined_down[num_bins], double &t_total_up, double &t_total_down, double t_up[2], double t_down[2], double v_effective_up[num_bins], double v_effective_down[num_bins], double &tdelay)
{
  char	last_loop;
  int	i, end_low_down, end_low_up, end_high_down, end_high_up, dma_type;
  ...
  double	v_low_extr_up[num_extr_bins], v_low_extr_down[num_extr_bins],
		...
		t, tscan, tstep, t_extr, tau, vol_DMA, Qsh, Qa, tres, v_high,
		v_low;
...
/* -------------------------------------------------------------------- */
void calc_mobility_limits(double v_combined_up[num_bins], double v_combined_down[num_bins], double T_K, double Zp_upper_limit[2][num_bins], double Zp_lower_limit[2][num_bins], double t_total_up, double t_total_down)
{
  double	Zp_star, Zp_ratio_high, Zp_ratio_low, t_total, time_step, Qsh;
  int	i, scan_up, bin, dma_type;
  char	ratio_file[20];
...
/* -------------------------------------------------------------------- */
void calc_rough_charge_prob(double Dp_nm, double T_K, double charge_prob[max_chg], int chgs)
{
...
/* -------------------------------------------------------------------- */
double Potential(double b, double T_K, double a, int p)
{
  double y, result;
...
/* -------------------------------------------------------------------- */
double alpha(double KE_ls, double E_ls, double a, int p, double ls)
{
...
/* -------------------------------------------------------------------- */
double simpson(double aa, double bb,double eps, double(*f)(double, double, double, int), double T_K, double a, int p)
{

  double simp, t1, t2, s1, ep, pp, hh, b;
  int n, k;
...
/* -------------------------------------------------------------------- */
void calc_charge_prob(double Dp_nm, double T_K, double P_atm, double charge_prob[max_chg], int charges_in)
{
...
/* -------------------------------------------------------------------- */
double get_cnc_counting_eff(double Dp_m, int charge)
{

  int		cnc_type, i;
  double	cnc_eff, delta_T_cnc, a, D1_nm, D2_nm, D0_nm,
		Dp_nm, T_sat, T_cond;
...
/* -------------------------------------------------------------------- */
double get_tubing_penetration(double single_length, double Q, double D)
{
...
/* -------------------------------------------------------------------- */
double get_efficiences(double Dp_nm, double vis, double Cc)
{
...
/* -------------------------------------------------------------------- */
void get_diameter_bins(double volt_avg_up[num_bins], double volt_avg_down[num_bins], int bins, double Dp_m[3][num_bins], int scan_up)
{
...
/* -------------------------------------------------------------------- */
void calc_inversion_matrices(double Zp_high_v_min, double Zp_low_v_max, int mat_num)
{
  ...
  ofstream mat_par_inv_out(mat_par_inv_file, ios::beg | ios::trunc);
...
//	cout<<endl<<"Creating down scan matrix";
    ...
    ofstream dia_lv_out(mat_dia_lv_file[scan_up], ios::beg | ios::trunc);
    ...
    ofstream inv_dia_out(mat_dia_inv_file[scan_up], ios::beg | ios::trunc);
    ...
    ofstream matrix_out(matrix_file[scan_up], ios::beg | ios::trunc);
