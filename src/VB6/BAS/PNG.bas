...
Private Declare Function crt_fclose Lib "vbpng1.dll" (ByVal stream As Long) As Long
Private Declare Function crt_fread Lib "vbpng1.dll" (buffer As Any, _
    ByVal nsize As Long, ByVal nCount As Long, ByVal stream As Long) As Long
Private Declare Function crt_malloc Lib "vbpng1.dll" (ByVal nsize As Long) As Long
...
Public Function LoadFormAsPNG(sPNGFileName As String, _
    pct As Form) As Boolean
    'InIDEHack
...
Public Function SaveFormAsPNG(pct As Form, sPNGFileName As String) As Boolean
    'InIDEHack
    ...
    nWidth = pct.ScaleX(pct.ScaleWidth, pct.ScaleMode, vbPixels)
    nHeight = pct.ScaleY(pct.ScaleHeight, pct.ScaleMode, vbPixels)
    
    If nWidth <= 0 Or nHeight <= 0 Then Exit Function
    ...
    If png_ptr = 0 Then crt_fclose FP: Exit Function

    info_ptr = png_create_info_struct(png_ptr)
    If info_ptr = 0 Then crt_fclose FP: Exit Function
...
End Function
Private Sub SaveAs(FQFN$)
  Dim sFile As String, i As Integer, sExt As String
  
'  sFile = FileSaveDialog(frmCaptureForm.picImage.hwnd, "Portable Network Graphics (*.png)|*.png|JPEG Image (*.jpg)|*.jpg|BMP Image (*.bmp)|*.bmp|GIF Image (*.gif)|*.gif", "Save Image as", g_sDefaultFolder, FQFN$)
