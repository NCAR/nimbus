...
Begin VB.Form frmUCSEsim 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(1)   =   "UCSEsim.frx":001C
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "Frame9"
      Tab(1).Control(1)=   "txtER2filename"
      Tab(1).Control(2)=   "cmdOpenER2"
      Tab(1).Control(3)=   "chkWriteToFile"
      Tab(1).Control(4)=   "txtUCSEfile"
      ...
      Begin VB.Frame Frame1 
         ...
         Top             =   480
         Width           =   2535
         Begin VB.CheckBox chkSubstituteCPUclock 
            Caption         =   "Substitute Computer Clock"
            Height          =   315
            Left            =   120
            TabIndex        =   185
            Top             =   2820
...
'      If X$ = "1010" Then 'if 10H appears in packet, it is doubled up to avoid confusion with DLE
'        Packet_2Ah = Left$(Packet_2Ah, Len(Packet_2Ah) - 1)
'      End If
    End If
  Loop Until x$ = "1003"         'DLE + ETX  at end of 2A packet
  If Len(Packet_2Ah) <> 49 Then
    x$ = x$
  End If
  ...
  For i = 1 To 5
    ...
    Do
      ...
      If Len(Packet_2Bh(i)) > 2 Then
        ...
        x$ = fHex2(Left$(x$, 1)) + fHex2(Right$(x$, 1))
'        If X$ = "1010" Then 'if 10H appears in packet, it is doubled up to avoid confusion with DLE
'          Packet_2Bh(i) = Left$(Packet_2Bh(i), Len(Packet_2Bh(i)) - 1)
'        End If
      End If
    Loop Until x$ = "1003"
    If Len(Packet_2Bh(i)) <> 35 Then
    x$ = x$
    End If
    If WriteHex Then Debug.Print ""
...
'    End If
  Next i
  Debug.Print Frame + 1; Len(Packet_2Ah); Len(Packet_2Bh(1)); Len(Packet_2Bh(2)); Len(Packet_2Bh(3)); Len(Packet_2Bh(4)); Len(Packet_2Bh(5))
...
Dim out$, i%
  ...
  If chkRepeat.Value = 0 Then ReadNextFrameFile
  cmdRefresh_Click
  
  out$ = FF + FF + Zero + Zero + DLE + Packet_2Ah + DLE + ETX
  out$ = out$ + DLE + Packet_2Bh(1) + DLE + ETX
  
  MSComm1.Output = out$
  Timer2.Enabled = True  'Set up Timer2 to send remaining 2Bh packets
  Timer2_Start = True    'Set TRUE for first entry into Timer2 routine
  DoEvents
...
Dim P2Ah As String, x$, HR!

' Check to make sure no double 10H
  P2Ah = ""   'Packet_2Ah with 10H removed
  For i = 1 To Len(Packet_2Ah) Step 2
    x$ = Mid$(Packet_2Ah, i, 2)
    x$ = fHex2(Left$(x$, 1)) + fHex2(Right$(x$, 1))
    If x$ = "1010" Then
      P2Ah = P2Ah + Mid$(Packet_2Ah, i, 1)
    Else
      P2Ah = P2Ah + Mid$(Packet_2Ah, i, 2)
    End If
  Next i
' Read CpK(2),GMT(9),LAT(10),AA(2),LON(11),GS(5),TA(3),Zgps(2),CC1(2)
'      1-2    3-11   12-21   22-23 24-34   35-39 40-42 43-44   45-46
...
' Read Status Byte CC1
  High = Asc(Mid$(P2Ah, 46, 1))  ' Get High Byte
  Low = Asc(Mid$(P2Ah, 47, 1)) ' Get Low Byte
  CC1 = 256 * High + Low
  txtCC1.Text = BitString(CC1)

' Decode GMT
  If chkSubstituteCPUclock.Value = 1 Then
    If Right$(Time, 2) = "PM" Then
      HR = Val(Left$(Time, 2)) + 12
      HHMMSS$ = Format(HR, "00") + Mid$(Time, 4, 2) + Mid$(Time, 7, 2) + ".00"
    Else
      HHMMSS$ = Left$(Time, 2) + Mid$(Time, 4, 2) + Mid$(Time, 7, 2) + ".00"
    End If
    Mid$(P2Ah, 4, 9) = HHMMSS$
    Mid$(Packet_2Ah, 4, 9) = HHMMSS$
  End If
  HHMMSS$ = Mid$(P2Ah, 4, 9)
  HH_MM_SS$ = Mid$(P2Ah, 4, 2) & ":" & Mid$(P2Ah, 6, 2) & ":" & Mid$(P2Ah, 8, 5)
  txtHHMMSS = HH_MM_SS$
  UTks = 3.6 * Val(Mid$(P2Ah, 4, 2)) + 0.06 * Val(Mid$(P2Ah, 6, 2)) + Val(Mid$(P2Ah, 8, 5)) / 1000#
  txtUTks = Str(UTks)
...
' Decode Latitude
  ...
  If Mid$(P2Ah, 13, 1) = "S" Then Latitude = -Latitude
...
' Read Longitude
  ...
  If Mid$(P2Ah, 25, 1) = "W" Then Longitude = -Longitude
...
' Status Word 2 (deg)
  P2Bh = Packet_2Bh
  Select Case Len(P2Bh)
  
  Case Is < 33
'    Debug.Print "P2Bh has " + Str(Len(P2Bh)) + " bytes! " + fHexString(P2Bh)
    Exit Sub
  Case 33
  Case Is > 33
    If InStr(1, P2Bh, Chr(16) + Chr(16)) > 0 Then
'      Debug.Print "P2Bh has " + Str(Len(P2Bh)) + " bytes! " + fHexString(P2Bh)
      Do
        i = InStr(1, P2Bh, Chr(16) + Chr(16))
        If i > 0 Then P2Bh = Left$(P2Bh, i) + Right$(P2Bh, Len(P2Bh) - i - 1)
      Loop Until i = 0
'      Debug.Print "P2Bh has " + Str(Len(P2Bh)) + " bytes! " + fHexString(P2Bh)
      If Len(P2Bh) < 33 Then Exit Sub
    Else
      If Hex$(Asc(Mid$(P2Bh, Len(P2Bh) - 1, 1))) + Hex$(Asc(Right$(P2Bh, 1))) = "DA" Then
      Else
'        Debug.Print "P2Bh has " + Str(Len(P2Bh)) + " bytes! " + fHexString(P2Bh)
      End If
      Exit Sub
  ...
  High = Asc(Mid$(P2Bh, 32, 1))  ' Get High Byte
  Low = Asc(Mid$(P2Bh, 33, 1)) ' Get Low Byte
  CC2(n) = 256 * High + Low
  txtCC2.Text = BitString(CC2(n))
' 1 T, 2 P, 3 TAS, 4 pALT, 5 DA, 6 Roll, 7 TH, 8 080-1
' 9 Pitch, 10 AA, 11 SA, 12 ADC, 13 Wind, 14 LSS, 15 DS, 16 080-1
' Outside air temperature (C)
  OAT = fGetN(P2Bh, 2, RBflag) * 1024# / 32768#
...
' Wind Direction (deg)
  WindDir = fGetN(P2Bh, 28, RBflag) * 180# / 32768#
...
Static iP%
  ...
  End If
  If Timer2_Start Then iP = 2 Else iP = iP + 1
  out$ = DLE + Packet_2Bh(iP) + DLE + ETX
  MSComm1.Output = out$
  DoEvents
  Timer2_Start = False
  If iP < 5 Then Timer2.Enabled = True
  If iP = 5 Then Timer1.Enabled = True
